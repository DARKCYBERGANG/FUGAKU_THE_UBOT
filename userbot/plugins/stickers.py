...
CUSTOM_STICKER_NAME =str(CUSTOM_STICKER_PACK_NAME) if CUSTOM_STICKER_PACK_NAME else "My Master's Volume Pack One"
CUSTOM_ANIME_PACK = str(CUSTOM_ANIMATED_PACK_NAME) if CUSTOM_ANIMATED_PACK_NAME else "My Master's Animated Pack"
...
async def _(event):
    ...
    #packname = f"FUGAKU PACK"
    #packshortname = f"FUGAKU_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 1263617196:
        packname = f"@CYBERMAFIA Packs 🎭"
        packshortname = "CYBERMAFIA"
    else:
        packname = f"{user.username}'s {pack}"
        packshortname = f"FUGAKU_{userid}_Pack"
    await event.edit("`Wew ! I Love That Sticker ! Mind If i Kang It ?`")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@FUGAKUOT.png"
    ...
    if is_a_s:
        ...
        if userid == 813878981:
            packname = f"CYBERMAFIA Ka Pack"
            packshortname = "WE ALL ARE PRO"
        else:
            packname = f"{user.username}'s {pack}"
            packshortname = f"FUGAKU_{userid}" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                   ...
                   # else:
                       # packname = f"Vol._{pack}_FUGAKU{userid}"
                        #packshortname = f"Vol._{pack}_FUGAKU_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
